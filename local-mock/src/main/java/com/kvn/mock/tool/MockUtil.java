...
public class MockUtil {
	...
	public static <T> T randomInstanceOfNonCollection(Class<T> clazz) throws Exception {
		...
		if (clazz == BigDecimal.class) {
			return (T) new BigDecimal(RANDOM.nextFloat());
		}
		// 日期
		if (clazz == Date.class) {
			return (T) new Date();
		...
		try {
			for (Class<?> type = clazz; type != Object.class; type = type.getSuperclass()) {
				...
				for (Method method : methods) {
					...
					String getMethodName = parameterTypes[0] == boolean.class ? "is" + method.getName().substring(3) :"g" + method.getName().substring(1);
